...
export default function OwnerContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setState('loading')
    try {
      // Get the Altcha payload from the hidden input populated by the widget.
      const form = e.currentTarget as HTMLFormElement
      const altchaPayload = (form.elements.namedItem('altcha') as HTMLInputElement | null)?.value ?? null
  ...
  return (
    <>
      {/* Altcha self-hosted CAPTCHA — runs proof-of-work automatically on load */}
      <Script src="https://cdn.jsdelivr.net/npm/altcha/dist/altcha.min.js" strategy="lazyOnload" type="module" />
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {/* Altcha widget — invisible PoW, auto-solves on load, writes to hidden input named "altcha" */}
        {/* @ts-expect-error — altcha-widget is a custom element registered by the CDN script */}
        <altcha-widget
